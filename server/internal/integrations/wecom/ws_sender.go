...
type wsSender struct {
	...
	// Bumped inside the writer section (wmu), so it is the wire order by
	// construction, and it is what
...
func (s *wsSender) write(frame map[string]any) error {
	...
	// Extract the trace fields out here, emit them in there. The writer
	// section is the point at which the ping loop, agent replies, inbox pushes
	// and stream frames become ordered, so a record taken inside it matches the
	// wire by construction, while one taken outside is only correlated with it
	// — a goroutine can emit its line and be descheduled before it takes the
	// writer, and the log then names the wrong frame as first. Extraction is
	// the expensive half (a regexp redaction pass and a rune-wise cut over the
	// message body) and needs no such guarantee, so it stays out here.
...
//
// seq is bumped here, inside the writer section, so it is the wire order by
// construction. t is the trace record its caller extracted before taking the
// writer; nil when tracing is off, which is the only cost this path pays then.
