...
type aibotMsgCallback struct {
	...
	// still takes the unsupported-kind receipt path, untouched by this file,
	// so teaching the adapter to read one stays a separate change. mixedItem
	// does carry a voice run, because dropping it would lose a spoken
	// sentence out of the middle of a message whose other runs are read.
}
...
func channelMessageFromCallback(botID, botDisplayName string, mc aibotMsgCallback, text, reqID string) channel.InboundMessage {
	...
	// Passing the resolved body straight through keeps p2p exactly where it
	// was before CommandText was set here.
	command := text
	if chatType == channel.ChatTypeGroup {
		command = stripLeadingMentions(text, botDisplayName)
	}
	...
	return channel.InboundMessage{
		...
		//
		// That source is the RESOLVED body, not mc.Text.Content: a 图文混排
