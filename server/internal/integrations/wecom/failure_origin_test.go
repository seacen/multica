...
// failedLine is the "that run did not go through" copy these tests read back.
// It was a package constant until the copy pack made every user-visible line a
// copyPack field, and it is taken from the default pack because every rig here
// leaves the language lookup nil — a reader with no profile, which is what
// localeFor answers the deployment default for.
var failedLine = copyFor(DefaultLocale).StreamFailed
...
func TestAWecomRunsFailureStillReachesTheAskerAfterThePromise(t *testing.T) {
	...
	if len(got) != 1 || got[0] != failedLine {
		t.Fatalf("the asker read %q, want exactly [%q] — they were promised a separate reply "+
			"and the failure of their own question never arrived", got, failedLine)
	}
}
...
func TestAWecomRunsFailureStillClosesTheBubbleItOpened(t *testing.T) {
	...
	if len(frames) != 2 || frames[1]["finish"] != true || frames[1]["content"] != failedLine {
		t.Fatalf("the bubble was left as %v, want it sealed with %q — the asker is watching a "+
			"spinner for a run that is already dead", frames, failedLine)
	}
}
...
func TestAFailureWithNoTaskIDIsStillDelivered(t *testing.T) {
	...
	if got := pushedTexts(t, rig.conn); len(got) != 1 || got[0] != failedLine {
...
func TestTheOriginOfARetryCloneIsItsParentsBatch(t *testing.T) {
	...
	if got := pushedTexts(t, rig.conn); len(got) != 1 || got[0] != failedLine {
		t.Fatalf("the asker read %q, want [%q]", got, failedLine)
	}
}
