...
// TestTheReadChunkStaysInTheRangeItWasChosenFor pins mediaStreamChunk, which
// nothing else does: every other test in this file measures against the
// constant, so it walks wherever the constant walks and a one-byte buffer
// would leave the suite green. Correctness really is chunk-size independent —
// that is what the sweep above proves — so what needs a guard is the reason
// the number was picked, which is the whole of why the streaming path exists.
//
// Too small and the 100 MiB ceiling turns into hundreds of thousands of reads
// and CryptBlocks calls, which is the cost the streaming path was meant to
// avoid paying. Too large and the buffer is back in the same order of
// magnitude as the file, which is the memory the streaming path was meant to
// avoid holding — eight concurrent ingests times the buffer is the number
// that matters, not one.
func TestTheReadChunkStaysInTheRangeItWasChosenFor(t *testing.T) {
	t.Parallel()
	if mediaStreamChunk < 64<<10 {
		t.Errorf("mediaStreamChunk = %d; below 64 KiB a %d-byte file is read in more than a thousand syscalls", mediaStreamChunk, maxMediaBytes)
	}
	if mediaStreamChunk > 4<<20 {
		t.Errorf("mediaStreamChunk = %d; above 4 MiB the buffer stops being incidental once several ingests overlap", mediaStreamChunk)
	}
	if mediaStreamChunk%aes.BlockSize != 0 {
		t.Errorf("mediaStreamChunk = %d, which is not a multiple of the %d-byte cipher block; the decryptor then carries a partial block across every read for no reason", mediaStreamChunk, aes.BlockSize)
	}
}
