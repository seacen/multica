...
// aibot_upload_media_init, the body.total_size row), and 20MB for a plain file
// is the widest any kind may be. See maxMediaUploadBytes, which cites the same
// place. The 50 MB the chunk arithmetic allows is what the FRAMING can
// describe, not what the platform takes.
func TestSplitMediaChunks_RejectsWhatCannotBeUploaded(t *testing.T) {
	...
	if maxMediaUploadBytes != 20<<20 {
		t.Errorf("maxMediaUploadBytes = %d, want %d — the source is WeCom's documented total_size limit", maxMediaUploadBytes, 20<<20)
	}
}
