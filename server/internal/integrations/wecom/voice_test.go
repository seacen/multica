...
func TestVoiceTranscriptIsTreatedAsText(t *testing.T) {
	...
	msg := channelMessageFromCallback("bot-1", "", mc, copyFor(DefaultLocale), body, "req-1")
...
func TestSpokenIssueCommandIsACommand(t *testing.T) {
	...
	spoken, _ := mc.ownText()
	msg := channelMessageFromCallback("bot-1", "", mc, copyFor(DefaultLocale), spoken, "req-1")
	if !msg.SkipAgentRun {
...
// The downloadable kinds still take the receipt path — this PR does not add
// media ingestion.
