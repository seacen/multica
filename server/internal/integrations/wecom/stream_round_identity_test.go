...
func TestTheRetryLookupIsNotPaidForOnEveryAnswer(t *testing.T) {
	...
	// One read, and it is the origin gate's: every ending pays that one
	// before anything WeCom-side happens. What must not appear is a SECOND
	// read for the same ending — the retry-clone lookup asking again for the
	// row the gate is already holding.
	if rig.q.taskGets != 1 {
		t.Fatalf("an answer that matched its own round read %d task row(s), want 1 — the gate's", rig.q.taskGets)
	}
	// Nothing open now, so an unmatched ending must not add a read either.
	rig.answer(t, "a late stray", "task-3")
	if rig.q.taskGets != 2 {
		t.Fatalf("an ending for a session with no open round brought the total to %d, want 2 — one gate read each", rig.q.taskGets)
	}
}

// TestARetryCloneCostsNoExtraRead is the other half: the clone is the one
// case whose bubble is found by the batch owner rather than by the id on the
// event, and that owner is on the row the gate has already read. Reading it
// again would be a second query for one ending, on the answer path, for a
// column already in hand.
func TestARetryCloneCostsNoExtraRead(t *testing.T) {
	t.Parallel()
	rig := newBubbleRig(t)
	rig.ran(t, "REQ-R1", 1, "task-1")
	// FailTask's retry child: fresh id, inheriting the parent's input batch.
	...
	if rig.q.taskGets != 1 {
		t.Fatalf("a retry clone's answer read %d task row(s), want 1 — the gate's row already carries the batch owner", rig.q.taskGets)
	}
	frames := rig.conn.streamFrames(t)
	if len(frames) < 2 || frames[len(frames)-1]["finish"] != true {
		t.Fatalf("the retry's answer did not seal the bubble the first attempt opened: %v", frames)
	}
}
