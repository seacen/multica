...
func provablyNotSent(err error) bool {
	...
	case errors.Is(err, errPartiallySent):
		// An answer past the 20480-byte cap goes out as several frames, and a
		// failure on the second says nothing about the first, which the user
		// is already reading. Retrying such a send would repeat what landed.
		return false
