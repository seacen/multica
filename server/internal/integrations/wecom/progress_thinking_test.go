...
// question, which in a group is exactly what the closed tier exists for.
func TestThinkingIsForThePrincipalOnly(t *testing.T) {
	rig := levelRig(t)
	rig.ingestMessage(t, groupInbound("REQ-42", "CHAT-room", rig.principalSender))

	rig.typing.handleTaskMessage(taskMessageEvent(chatTaskID(), thinking("先把去年的报价单翻出来")))

	if frames := streamViews(t, &rig.conn.recordingConn); len(frames) != 1 {
		t.Fatalf("the group bubble took %d frames of thinking, want none", len(frames)-1)
	}
}
