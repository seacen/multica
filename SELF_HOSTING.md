...
> **Exception — WeCom (企业微信) smart bot must run single-replica.** A WeCom question is answered in a live bubble that opens the moment it arrives, and only the replica holding that bot's WebSocket connection can write into it: the stream handle lives in that process's memory, and the protocol accepts the bubble's frames on no other connection. Sealing the bubble with the answer is driven by the in-process `chat:done` event, which fires on whichever replica ran the task. If you run more than one backend replica with WeCom enabled (`MULTICA_WECOM_SECRET_KEY` set), a task that runs anywhere other than the socket holder leaves its bubble spinning until the five-minute guard closes it with "still working, I'll reply separately", and the answer then arrives as a separate message instead of in the bubble it was opened for. Until a bubble can be driven from a replica that does not hold the socket, run the WeCom-enabled backend as a single replica. Everything else (including the rollup scheduler above) is multi-replica safe.
