...
> **Exception — WeCom (企业微信) smart bot must run single-replica.** A WeCom question is answered in a live bubble that opens the moment it arrives, and everything needed to write into that bubble — the callback's `req_id`, the stream id, the chat it came from — is held in memory by the replica that received the callback. What stops another replica writing there is that handle, not the protocol: a second connection can refresh a stream the first one opened. Sealing the bubble with the answer is driven by the `chat:done` event, and the event bus is in-process and synchronous, so the event reaches subscribers only inside the replica that ran the task. Run more than one backend replica with WeCom enabled (`MULTICA_WECOM_SECRET_KEY` set) and a task that lands anywhere other than the socket holder is answered nowhere: the replica that ran it holds no WebSocket for that bot, so it drops the reply and logs `connection not ready on this replica`, while the socket holder — which never receives the event — closes the bubble itself after five minutes with "还在处理，完成后我再单独回复你" and then never sends the separate reply it has just promised. A failed or cancelled run is lost the same way, for the same reason. Until a bubble can be driven from a replica that does not hold the socket, run the WeCom-enabled backend as a single replica. Everything else (including the rollup scheduler above) is multi-replica safe.
